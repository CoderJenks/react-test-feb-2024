...
const CheckoutPage = () => {
  const { basketItems, addToBasket, removeFromBasket } = useContext(
    ShoppingBasketContext,
  );
  ...
  function BasketProductCard(item: BasketItemType) {
    return (
      <div
        className="BasketProductCard__Container"
        _data-testid={`ProductCard-${item.id}`}
      >
        <img
          className="BasketProductCard__Image"
          src={item.img}
          alt={item.name}
        />
        <div className="BasketProductCard__ProductDetails">
          ...
          <div className="BasketProductCard__Price">
            Price:{" "}
          ...
          <div>
            ...
            <div className="BasketProductCard__Quantity">
              <button
                className="BasketProductCard__RemoveItemButton"
                onClick={() => {
                  onRemovefromBasketClicked(item);
                }}
              >
                -
              </button>
              {item.quantity}
              <button
                className="BasketProductCard__AddItemButton"
                onClick={() => onAddToBasketClicked(item)}
              >
                +
              </button>
            </div>
  ...
  function BasketList({ items }: BasketListProps) {
    return (
      <>
        {items.map((item) => (
          <BasketProductCard
  ...
  return (
    <>
      <div className="CheckoutPage__Content">
        <h2 className="CheckoutPage__Header">Checkout</h2>
